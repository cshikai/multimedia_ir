...
### ================================= GRAPH MANAGEMENT FUNCTIONS ===================================== ###
    ...
    def merge_node(self, node_labels, node_attributes):
        with self.driver.session() as session:
            ...
            node_attributes = "{"+", ".join(
                [k+" : '"+str(node_attributes[k])+"'" for k in node_attributes.keys()])+"}"
    ...
    def merge_edge(self, source_node_label, source_node_attribute, target_node_label, target_node_attribute, relation_type, edge_attributes):
        ...
        # UNDIRECTED
        with self.driver.session() as session:
            source_attributes = "{"+", ".join([k+" : '"+str(
                source_node_attribute[k])+"'" for k in source_node_attribute.keys()])+"}"
            target_attributes = "{"+", ".join([k+" : '"+str(
                target_node_attribute[k])+"'" for k in target_node_attribute.keys()])+"}"
...
class GraphGenerator:
    ...
    def _generate_edges(self):
        for idx, triple in self.triple_df.iterrows():
            ...
            self.graph.merge_edge(source_node_label, source_node_attributes,
                                  target_node_label, target_node_attributes, relation_type, edge_attributes)


if __name__ == "__main__":
    # Dummy data for testing
    nodes = [["Obama", 1, "PPL"], ["USA", 2, "CTY"], ["MICHELLE", 3, "PPL"]]
    relations = [["Citizen", "RS1"], ["Husband", "RS2"]]
    triples = [[1, "RS1", 2], [1, "RS2", 3]]

    node_df = pd.DataFrame(
        nodes, columns=['node_name', 'node_id', 'entity_type'])
