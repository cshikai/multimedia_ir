...
class YOLOManager():
    ...
    def __init__(self):
        ...
        self.model = attempt_load(config['weights'], map_location=self.device) # Load YOLO Model
        self.imgsz = check_img_size(config['img_size'], s=self.model.stride.max())  # check img_size
        if self.half:
            self.model.half()  # to FP16
        if config['emb']:
            # Load pre-saved resnet model locally, so don't need to download each time container spins up
            self.classify_model = load_classifier(name='resnet101', n=2, local='resnet101.pt') 
    ...
    def detect(self, img_b64):
        ...
        #NMS
        pred = non_max_suppression(pred, config['conf_thres'], config['iou_thres'], classes=None, agnostic=config['agnostic_nms'])
